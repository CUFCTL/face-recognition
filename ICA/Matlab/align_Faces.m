%Script align_Faces (Robust version) %%%%%%%%%%%%%%% import this into C
%
...
if ispc == 1
    LabelDir = uigetdir();
    imgdir = strcat(LabelDir, '\train\');
    DestDir = strcat(LabelDir, '\AlignedFaceImages\');
...
elseif ismac == 1
    ...
    fprintf('Error determining computer type! Check align_Faces.m\n');
end
...
for i = START_ITER:(size(r,1) - END_ITER)
   ...
   I=imread(imgName);
     ...
     [height, width] = size(I);
     dxeyes = marks(i-MARK_OFF,3) - marks(i-MARK_OFF,1); %Wm --> changed all references to mark from i-2 to i-3 (similar to alignTestFaces)Check these. 
     dyeyes = marks (i-MARK_OFF,4) - marks(i-MARK_OFF,2); %Wm --> change described above on lines 48, 49, 51 - 53, 64, 65
